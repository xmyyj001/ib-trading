...
├── scripts/                   # 运营脚本
│   └── firestore/             # Firestore 初始化/配置脚本（init & setting）
...
### 2.5 Firestore 配置脚本（策略/资金更新）

`scripts/firestore/` 下的脚本是管理 Firestore 配置和策略 guardrail 的官方入口：

| 脚本 | 用途 | 示例 |
| --- | --- | --- |
| `init_firestore.py` | 全新环境或灾备恢复时，一次性写入 `config/*` 与账户文档。会使用脚本内置的 `config_data` 覆盖同名文档。 | `python scripts/firestore/init_firestore.py gold-gearbox-424413-k1` |
| `setting_firestore.py` | 日常调整 `config/common.exposure` 与 `strategies/{id}` guardrail（`allowed_symbols`, `max_notional` 等）。支持 `--dry-run` 与自定义权重。 | `python scripts/firestore/setting_firestore.py --project-id gold-gearbox-424413-k1 --overall-exposure 0.90 --testsignalgenerator-weight 0.33 --spy-macd-vixy-weight 0.33 --reserve-weight 0.34 --testsignalgenerator-max-notional 600000 --spy-macd-vixy-max-notional 600000` |

**新增或更新策略时的步骤：**
1. 在 `strategies/{strategy_id}` 文档中补齐 `enabled`, `allowed_symbols`, `max_notional` 等静态信息。
2. 使用 `setting_firestore.py --project-id <PROJECT_ID>`（可加自定义权重/限额参数）更新 `config/common.exposure.strategies`，确保权重之和 ≤ 1，常用命令如下：
...
```
3. 如需演练，先加 `--dry-run` 查看 payload，再去掉该参数真正写入。
4. 运行一次 `verify_trading.py --show-intents ...` 或 Cloud Run `/` dry-run，确认 Commander 的 Strategy Exposure 输出匹配新的权重。

> 除非需要完整重置 Firestore，否则优先使用 `setting_firestore.py`。`init_firestore.py` 会覆盖写入，共享环境下需谨慎操作。
