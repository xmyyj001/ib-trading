...
class Allocation(Intent):
    ...
    async def _cancel_stale_orders_async(self, open_orders, trades):
    ...
    async def _core(self):
        ...
        all_open_orders = await self._env.ibgw.reqOpenOrdersAsync()
        ...
        if 'NetLiquidation' not in account_values or not account_values['NetLiquidation']:
            raise ValueError(f"NetLiquidation not found for account {self._env.config['account']}.")
        ...
        if not self._dry_run:
            await self._cancel_stale_orders_async({o.contract.conId: o for o in all_open_orders}, trades.trades)
            OrderClass = LimitOrder
            orders = await trades.place_orders_async(OrderClass)
