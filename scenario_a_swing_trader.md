...
⚠️ **状态说明**：以下设计记录的是曾经探索的架构草案，目前尚未在 Google Cloud Run 上成功部署。`allocation` 意图与相关策略代码仍在重构阶段，实际试运行只覆盖了 `testsignalgenerator` 策略。
...
1. **完成 `allocation` 异步化**：在主线程/后台线程模型下重新实现，下游策略需要返回明确的目标仓位而非抽象权重。
2. **策略拆分**：将 `spymacdvixy`、`dummy` 等策略迁移到“策略即意图”范式，避免对中央调度器的依赖。
3. **持仓数据来源统一**：确认 Firestore 快照与 IB 实时数据的分工，必要时新增对账任务确保在途订单准确扣减。
